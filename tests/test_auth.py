...
class TestUpdateRole:
    def test_promote_to_nurse(self, client, db):
        ...
        response = client.put(
            f"/api/v1/auth/users/{patient.id}/role", json={"role": "nurse"}, headers=auth_header(admin)
        )
        assert response.status_code == 200
    ...
    def test_promote_to_doctor(self, client, db):
        ...
        nurse = create_test_user(db, role="nurse", email="nurse@test.com")
        response = client.put(
            f"/api/v1/auth/users/{nurse.id}/role", json={"role": "doctor"}, headers=auth_header(admin)
        )
        assert response.status_code == 200
    ...
    def test_demote_to_patient(self, client, db):
        ...
        response = client.put(
            f"/api/v1/auth/users/{doctor.id}/role", json={"role": "patient"}, headers=auth_header(admin)
        )
        assert response.status_code == 200
        assert response.json()["role"] == "patient"
    ...
    def test_cannot_change_own_role(self, client, db):
        admin = create_test_user(db, role="admin")
        response = client.put(
            f"/api/v1/auth/users/{admin.id}/role", json={"role": "patient"}, headers=auth_header(admin)
        )
        assert response.status_code == 400
    ...
    def test_invalid_role_rejected(self, client, db):
        ...
        response = client.put(
            f"/api/v1/auth/users/{user.id}/role", json={"role": "superadmin"}, headers=auth_header(admin)
        )
        assert response.status_code == 422

    def test_user_not_found(self, client, db):
        admin = create_test_user(db, role="admin")
        response = client.put("/api/v1/auth/users/nonexistent/role", json={"role": "nurse"}, headers=auth_header(admin))
    ...
    def test_non_admin_cannot_change_roles(self, client, db):
        ...
        response = client.put(
            f"/api/v1/auth/users/{patient.id}/role", json={"role": "doctor"}, headers=auth_header(nurse)
        )
        assert response.status_code == 403
    ...
    def test_role_change_generates_audit(self, client, db):
        ...
        client.put(f"/api/v1/auth/users/{patient.id}/role", json={"role": "nurse"}, headers=auth_header(admin))
        response = client.get("/api/v1/audit?action=update&resource_type=user", headers=auth_header(admin))
        data = response.json()
        assert data["total"] >= 1
...
class TestDeactivateActivate:
    def test_deactivate_user(self, client, db):
        ...
        response = client.put(f"/api/v1/auth/users/{user.id}/deactivate", headers=auth_header(admin))
        assert response.status_code == 200
    ...
    def test_activate_user(self, client, db):
        ...
        client.put(f"/api/v1/auth/users/{user.id}/deactivate", headers=auth_header(admin))
        response = client.put(f"/api/v1/auth/users/{user.id}/activate", headers=auth_header(admin))
        assert response.status_code == 200
        assert response.json()["is_active"] is True

    def test_cannot_deactivate_self(self, client, db):
        admin = create_test_user(db, role="admin")
        response = client.put(f"/api/v1/auth/users/{admin.id}/deactivate", headers=auth_header(admin))
        assert response.status_code == 400

    def test_deactivate_not_found(self, client, db):
        admin = create_test_user(db, role="admin")
        response = client.put("/api/v1/auth/users/nonexistent/deactivate", headers=auth_header(admin))
        assert response.status_code == 404

    def test_non_admin_cannot_deactivate(self, client, db):
        nurse = create_test_user(db, role="nurse", email="nurse@test.com")
        patient = create_test_user(db, role="patient", email="patient@test.com")
        response = client.put(f"/api/v1/auth/users/{patient.id}/deactivate", headers=auth_header(nurse))
        assert response.status_code == 403
