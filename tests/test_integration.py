...
if PG_URL.startswith("postgresql"):
    ...
    class TestPostgresPatients:
        def test_create_and_read_patient(self, pg_client, pg_db):
            nurse = User(
                email="nurse@pg.com", hashed_password=hash_password("test1234"), full_name="PG Nurse", role="nurse"
            )
            pg_db.add(nurse)
            pg_db.commit()
            pg_db.refresh(nurse)
        ...
        def test_patient_list_pagination(self, pg_client, pg_db):
            nurse = User(
                email="nurse2@pg.com", hashed_password=hash_password("test1234"), full_name="PG Nurse 2", role="nurse"
            )
            pg_db.add(nurse)
            pg_db.commit()
            pg_db.refresh(nurse)
    ...
    class TestPostgresTriage:
        def test_triage_persists_to_pg(self, pg_client, pg_db):
            nurse = User(
                email="tri@pg.com", hashed_password=hash_password("test1234"), full_name="Tri Nurse", role="nurse"
            )
            pg_db.add(nurse)
            pg_db.commit()
            pg_db.refresh(nurse)
    ...
    class TestPostgresAuth:
        ...
        def test_json_columns_work_with_pg(self, pg_client, pg_db):
            ...
            nurse = User(
                email="json@pg.com", hashed_password=hash_password("test1234"), full_name="JSON Nurse", role="nurse"
            )
            pg_db.add(nurse)
            pg_db.commit()
            pg_db.refresh(nurse)
